...
export class ColorListController {
    ...
    async saveAnswer(@Param('problemID') problemID: number, @Body() detail: MyColors[]): Promise<number> {
    ...
    @Get("/color-list")
    async getList(): Promise<MyColors[]> {
        return this.colorRepository.getList();
    }
